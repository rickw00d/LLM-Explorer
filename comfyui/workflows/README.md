...
2. 逐一各跑一次（DGX Spark 記憶體雖大，但一次跑一個重模型最穩、也最好對照時間）。
...
> 進階：可把三個模板合到同一張畫布，共用同一個「CLIP Text Encode（prompt）」與固定 seed 的
> primitive 節點，分別接到三個 sampler，一次 Queue 跑完三組。做好後 **Save (API/JSON)** 匯出，
> 放回本資料夾（例如 `compare_ltx_wan_h3.json`）方便重複使用。

## 存放你自己的 workflow
把在 ComfyUI 裡調好、匯出的 `.json` 放到這個資料夾，之後用 ComfyUI 的
**Workflow → Open** 載入即可重現。
