...
# ComfyUI binds 127.0.0.1:8188; the compare server binds 0.0.0.0:8890 by default
...
if curl -sf -o /dev/null --max-time 2 "$COMFY_URL/system_stats" 2>/dev/null; then
  ...
  # Never let a ComfyUI failure take the compare server down with it: the page is
  # still worth serving, and it shows ComfyUI as offline.
  if "$COMFY_SH"; then
    ...
    echo ">> ComfyUI failed to start — carrying on; generation will fail until it is up."
  fi
else
...
cd "$SCRIPT_DIR"
# run.sh picks up .token and execs, so this PID is the real server process.
...
if [[ -n "${COMPARE_TOKEN:-}" || -f "$TOKF" ]]; then
